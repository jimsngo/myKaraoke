...
maximize_volume() {
    ...
    local PEAK_DB=$(ffmpeg -nostdin -i "$INPUT_FILE" -af "volumedetect" -vn -f null - 2>&1 | grep "max_volume:" | awk '{print $5}')
    if [ -z "$PEAK_DB" ]; then return; fi
    local GAIN=$(echo "scale=2; -1 * ($PEAK_DB) - $CEILING" | bc)
    ffmpeg -nostdin -y -nostdin -i "$INPUT_FILE" -af "volume=${GAIN}dB" -c:v copy "$OUTPUT_FILE"
}
...
strip_audio() {
    ...
    ffmpeg -nostdin -y -nostdin -i "$INPUT_FILE" -an -c:v copy "$OUTPUT_FILE"
}